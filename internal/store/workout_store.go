...
//decoupling db w/ interface so we can use this name
type WorkoutStore interface {
	...
	DeleteWorkout(id int64) error
		...
		//func, receiver, func name , parameter, return type
func(pg *PostgresWorkoutStore) CreateWorkout(workout *Workout)(*Workout, error){
		...
		err = tx.QueryRow(query, workout.Title, workout.Description, 
...
func(pg *PostgresWorkoutStore) GetWorkoutByID(id int64)(*Workout, error){
	...
	defer rows.Close() //close any lingering query
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	//know how many rows were affected coz we won't know if they updated a non existing value
	...
	//entries
	// Full on patch update (put) - delete then change
	//reassigning error that's why we can do _, err =
